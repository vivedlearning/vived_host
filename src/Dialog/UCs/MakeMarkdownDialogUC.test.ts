...
function makeTestRig() {
  ...
  const uc = makeMakeMarkdownDialogUC(appObjects.getOrCreate("dialog"));
...
describe("Make Markdown Dialog UC", () => {
  it("Gets the UC", () => {
    const { uc, appObjects } = makeTestRig();
    expect(MakeMarkdownDialogUC.get(appObjects)).toEqual(uc);
  });
