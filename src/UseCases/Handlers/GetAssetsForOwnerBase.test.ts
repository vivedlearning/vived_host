...
import { GetAssetsForOwnerBase } from './GetAssetsForOwnerBase';

function makeTestRig() {
  const hostHandler = makeHostHandler();
  const getAssetMetaBase = new GetAssetsForOwnerBase(hostHandler);
...
describe('Get Assets for owner Base Handler', () => {
  ...
  it('Throws an error if the action is not overwritten', () => {
    ...
    expect(() => getAssetMetaBase.action('anOwner', jest.fn())).toThrowError();
  });
  ...
  it('Triggers the action for v1', () => {
    ...
    expect(getAssetMetaBase.action).toBeCalledWith('anOwner', mockCallback);
  });
