...
import { makePostNewAssetUC, NewAssetDTO, PostNewAssetUC } from './PostNewAssetUC';
...
describe('JSON Requester', () => {
  ...
  it('Rejects if the get auth token fails', () => {
    ...
    return expect(uc.doPost(dto)).rejects.toEqual(new Error('Some Auth token error Error'));
  });
  ...
  it('Rejects if the upload fails', () => {
    ...
    return expect(uc.doPost(dto)).rejects.toEqual(new Error('Some upload Error'));
  });
});
