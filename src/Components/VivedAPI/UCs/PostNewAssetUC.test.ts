...
import {
  makeMockSignedAuthTokenUC
} from "../Mocks/MockSignedAuthToken";
...
describe("JSON Requester", () => {
  ...
  it("Resolves with the new Asset ID", async () => {
    ...
    const assetID = await uc.doPost(dto);

    expect(assetID).toEqual("newAssetID");
  });
