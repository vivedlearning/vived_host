...
export abstract class GetAssetsForOwnerUC extends HostAppObjectUC {
  static type = "GetAssetsForOwnerUC";

  abstract getAssets(ownerID: string): Promise<OwnerAssetDTO[]>;

  static get(appObjects: HostAppObjectRepo): GetAssetsForOwnerUC | undefined {
    return getSingletonComponent(GetAssetsForOwnerUC.type, appObjects);
  }
}

export function makeGetAssetsForOwnerUC(
  appObject: HostAppObject
): GetAssetsForOwnerUC {
  return new GetAssetsForOwnerUCImp(appObject);
}

class GetAssetsForOwnerUCImp extends GetAssetsForOwnerUC {
