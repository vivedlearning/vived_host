...
import { makeGetAssetsForOwnerFromAPIUC, GetAssetsForOwnerFromAPIUC } from './GetAssetsForOwnerFromAPIUC';
...
describe('Get Assets for Owner UC', () => {
  ...
  it('Warns and rejects if the JSON Request rejects', () => {
    ...
    return expect(uc.getAssets('owner')).rejects.toEqual(new Error('Some JSON Error'));
  });
  ...
  it('Requests the JSON with the expected url', async () => {
    ...
    expect(postURL.toString()).toEqual('https://api.vivedlearning.com/assets/group/owner?page=1&itemsPerPage=100');
  });
  ...
  it('Rejects if the get auth token fails', () => {
    ...
    return expect(uc.getAssets('owner')).rejects.toEqual(new Error('Some Auth token error Error'));
  });
});
