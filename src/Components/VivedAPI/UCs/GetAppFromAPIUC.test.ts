...
describe('Patch Asset Is Archived UC', () => {
  ...
  it('Warns and rejects if the JSON Request rejects', () => {
    ...
    return expect(uc.getApp('app1', '1.2.3')).rejects.toEqual(new Error('Some JSON Error'));
  });
  ...
  it('Requests the JSON with the expected url', async () => {
    ...
    expect(postURL.toString()).toEqual('https://api.vivedlearning.com/apps/app1/1.2.3?function_version=2');
  });
  ...
  it('Rejects if the get auth token fails', () => {
    ...
    return expect(uc.getApp('app1', '1.2.3')).rejects.toEqual(new Error('Some Auth token error Error'));
  });
});
