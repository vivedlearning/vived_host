...
import { FetchAssetMetaUC, makeFetchAssetMetaUC } from "./FetchAssetMetaUC";
...
function makeTestRig() {
  ...
  const uc = makeFetchAssetMetaUC(appObjects.getOrCreate("ao"));
...
describe("Fetch Asset Meta", () => {
  ...
  it("Gets the singleton", () => {
    ...
    expect(FetchAssetMetaUC.get(appObjects)).toEqual(uc);
  });
  ...
  it("Requests with the expected options", async () => {
    ...
    expect(mockRequest.doRequest).toBeCalled();
    const options = mockRequest.doRequest.mock
  ...
  it("Rejects if the request rejects", () => {
    ...
    return expect(uc.doFetch("asset1")).rejects.toEqual(
