import { AppSandboxEntity, makeAppSandboxEntity } from "..";
import {
  HostAppObjectRepo,
...
describe("submitActivityAssetHandler", () => {
  let appObjects: HostAppObjectRepo;
  ...
  beforeEach(() => {
    appObjects = makeHostAppObjectRepo();
  ...
  it("should submit error when NewAssetUC is missing", (done) => {
    ...
    const handler = makeSubmitActivityAssetHandlerSandbox(appObjects);
