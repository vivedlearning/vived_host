...
import { makeZSpaceIsActivePM, ZSpaceIsActivePM } from "./ZSpaceIsActivePM";
...
function makeTestRig() {
  ...
  const pm = makeZSpaceIsActivePM(appObjects.getOrCreate("zSpace"));
...
describe("Emulate zSpace PM", () => {
  test("Initializes the VM", () => {
    ...
    expect(pm.lastVM).toEqual(zSpace.isActive);
  });
  ...
  it("Gets the singleton", () => {
    ...
    expect(ZSpaceIsActivePM.get(appObjects)).toEqual(pm);
  });
});
