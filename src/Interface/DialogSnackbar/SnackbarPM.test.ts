...
function makeTestRig() {
    ...
    const view = jest.fn();
    const pm = new SnackbarPM(snackbarUC, view);
    return { snackbarUC, pm, view };
}
...
test("PM updates when use case changes", () => {
    const { snackbarUC, view } = makeTestRig();
    snackbarUC.makeSnackbar("test");
    const lastVM = getLastModel(view);
...
test("PM disposes", () => {
    const { snackbarUC, pm, view } = makeTestRig();
    pm.dispose();
    snackbarUC.makeSnackbar("test");
    const lastVM = getLastModel(view);
    expect(lastVM?.message).toEqual(undefined);
...
test("Action and duration get set correctly", () => {
    const { snackbarUC, view } = makeTestRig();
    snackbarUC.makeSnackbar("test", {actionButtonText: "action", action: ()=>{const test=2}},2);
    const lastVM = getLastModel(view);
